...
def loss_fn(recon_x, x, mean, log_var):
    # Use MSE loss instead of BCE, more suitable for continuous features
    # Add gradient clipping and numerical stability
    Recon = torch.nn.functional.mse_loss(recon_x, x.detach(), reduction='mean')
    # Limit KLD loss to avoid over-regularization
    KLD = -0.5 * torch.sum(1 + log_var - mean.pow(2) - log_var.exp()) / x.size(0)
    KLD = torch.clamp(KLD, min=0.0, max=100.0)  # Clamp KLD range
    return (Recon + KLD)
...
class d2sc(torch.nn.Module):
    def __init__(self, data, n_T, betas, seenclasses, unseenclasses, attribute,  netG_con_model_path, device='cuda'):
        ...
        # Add defaults to opt if missing (skip when already present)
        if not hasattr(opt, 'gamma_mmd'):
            opt.gamma_mmd = 1.0
        if not hasattr(opt, 'gamma_center'):
            opt.gamma_center = 1.0

        # Add a learnable bias vector for each unseen class
        # Initialize unseen bias as a constant; trainable
    ...
    def update_D(self, x_0_real, con_0_real, att_0_real, gp_sum, label):
        ...
        # Add gradient clipping to prevent overly strong discriminator
        torch.nn.utils.clip_grad_norm_(self.netDec.parameters(), max_norm=1.0)
        torch.nn.utils.clip_grad_norm_(self.netD_x0.parameters(), max_norm=1.0)
        torch.nn.utils.clip_grad_norm_(self.netD_xt.parameters(), max_norm=1.0)
        torch.nn.utils.clip_grad_norm_(self.netD_xc.parameters(), max_norm=1.0)
    ...
    def update_G(self, x_0_real, con_0_real, att_0_real, label):
        ...
        # Add center loss (semantic centers mapped to visual space via S2V)
        ...
        torch.nn.utils.clip_grad_norm_(self.netE.parameters(), max_norm=1.0)
    ...
    def sample_from_model(self, att, progressive=False):
        ...
        with torch.no_grad():
            ...
            # If att belongs to unseen classes, add unseen bias
            # Assume att order aligns with unseenclasses (as in generate_syn_feature with unseenclasses)
            if att.shape[0] == len(self.unseenclasses):
                z = z + self.unseen_bias  # [num_unseen, latent_dim]
            # For batched generation, add bias by class index; can be extended further
...
d2sc = d2sc(data, n_T=opt.n_T, betas=(opt.ddpmbeta1, opt.ddpmbeta2), seenclasses=data.seenclasses,
