...
class d2sc_DRG(torch.nn.Module):
    ...
    def update_D(self, con_0_real, att_0_real, label):
        ...
        # Minimal change: optional mismatched-attribute negative sampling (disabled by default)
        if getattr(opt, 'drg_neg_sampling', 'none') == 'mismatch_att' and getattr(opt, 'drg_neg_ratio', 0.0) > 0:
            with torch.no_grad():
                n_neg = max(1, int(att_0_real.size(0) * float(opt.drg_neg_ratio)))
                idx = torch.randperm(att_0_real.size(0), device=att_0_real.device)[:n_neg]
                perm = torch.randperm(att_0_real.size(0), device=att_0_real.device)
                att_mismatch = att_0_real[perm][idx]
            criticD_mismatch_c0 = self.netD_c0(con_0_real.detach()[idx], att_mismatch).mean()
            criticD_fake = criticD_fake + self.gamma_x0 * float(opt.drg_neg_ratio) * criticD_mismatch_c0
    ...
    def update_G(self, con_0_real, att_0_real, label):
        ...
        # minimal, optional: timestep consistency to stabilize diffusion (default off)
        if getattr(opt, 'gamma_CON_step', 0.0) and float(opt.gamma_CON_step) > 0.0:
            con_step_consistency = F.mse_loss(con_t_fake, con_t_real)
            errG = errG + float(opt.gamma_CON_step) * con_step_consistency
...
d2sc_drg = d2sc_DRG(data, n_T=opt.n_T, betas=(opt.ddpmbeta1, opt.ddpmbeta2), seenclasses=data.seenclasses,
